...
class UserController extends Controller
{
    ...
    public function settings()
    {
        ...
        $user->load(['following' => function ($query) {
            $query->withCount('followers')->select('user_id', 'name', 'email', 'profile_image');
        }]);
    ...
    public function updateProfileImage(Request $request)
    {
        ...
        return response()->json(['message' => 'Profile image updated successfully', 'profile_image' => $imagePath], 200);
    }
    ...
    public function feed()
    {
        ...
        $posts = \App\Models\Post::whereIn('user_id', $followingIds)
            ->with(['images', 'likes', 'comments.user']) // Incluir relaciones de likes y comments con el usuario
    ...
    //     // Verificar si el usuario ya ha dado "me gusta" a este post
    //     $existingLike = Like::where('post_id', $post->id)->where('user_id', $user->id)->first();

    //     if ($existingLike) {
    //         // Si ya ha dado "me gusta", eliminar el "me gusta"
    //         $existingLike->delete();
    //         return response()->json(['message' => 'Like removed', 'like' => false], 200);
    //     } else {
    //         // Si no ha dado "me gusta", crear uno nuevo
    //         Like::create([
    //             'post_id' => $post->id,
    //             'user_id' => $user->id,
    //         ]);
    //         return response()->json(['message' => 'Post liked', 'like' => true], 201);
    //     }
    // }
